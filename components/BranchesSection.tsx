...
function HoursTable({
  ...
  const labels = locale === "no" ? DAYS_NO : DAYS_EN;
  return (
    <div className="rounded-xl border border-gray-100 overflow-hidden mt-3">
      {Object.entries(hours).map(([day, h], i) => (
        ...
        >
          ...
          {h?.closed ? (
            <span className="text-gray-400">{locale === "no" ? "Stengt" : "Closed"}</span>
          ) : (
...
export default function BranchesSection({ branches, locale }: BranchesSectionProps) {

...
export default function ReviewForm({ businessId }: ReviewFormProps) {
  ...
  const ratingLabels = [
    "",
    t("reviewForm.ratingPoor"),
    t("reviewForm.ratingFair"),
    t("reviewForm.ratingGood"),
    t("reviewForm.ratingVeryGood"),
    t("reviewForm.ratingExcellent"),
  ];

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (rating === 0)               { setError(t("reviewForm.errorRating"));  return; }
    if (comment.trim().length < 10) { setError(t("reviewForm.errorComment")); return; }
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? t("reviewForm.errorGeneric"));
      } else {
        setSuccess(true);
        setRating(0);
        setComment("");
      ...
      setError(t("reviewForm.errorGeneric"));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (success) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
        ...
        <p className="font-semibold text-green-900 mb-1">{t("reviewForm.successTitle")}</p>
        <p className="text-sm text-green-700">{t("reviewForm.successBody")}</p>
        <button
          onClick={() => setSuccess(false)}
          className="mt-4 text-sm text-green-700 underline hover:no-underline"
        >
          {t("reviewForm.writeAnother")}
        </button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-50 border border-gray-200 rounded-xl p-6 space-y-5">
      <h4 className="font-semibold text-gray-900">{t("reviewForm.title")}</h4>
        ...
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((star) => (
            <button
              ...
              aria-label={t("reviewForm.rateStar", { star })}
            >

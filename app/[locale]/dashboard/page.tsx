...
export default async function DashboardOverviewPage({
  ...
  if (!business) {
    return (
      <div>
        <PageHeader title={t("title")} subtitle={t("overview.welcome", { name: session!.user!.name ?? session!.user!.email ?? "" })} />
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title={t("title")}
        subtitle={t("overview.welcome", { name: session!.user!.name ?? session!.user!.email ?? "" })}
      />

      {/* Status + primary actions */}
      <div className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <StatusRow status={business.status} t={t} />
      ...
      {/* Stats — balanced 2×2 / 1×4 grid (no orphan card) */}
...
function StatusRow({ status, t }: { status: string; t: (k: string) => string }) {
